...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Seja Bem Vindo (a) @${num.split('@')[0]}\nEspero que você se divirta no (a) *${mdata.subject}*`
				...
				teks = `O (a) @${num.split('@')[0]}Ja Foi Tardekkkkkkkkkkk👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Criando... Por Favor, Aguarde no maximo 10 Segundos, caso nao obtenha Tente Novamente!!!⌛',
				...
				only: {
					...
					admin: '[❗] VAI SE FUDER MEMBRO COMUM🖐🏻 ESSE COMANDO SÓ PODE SER USADO POR ADMINISTRADORES!!!!!!!!!!❌',
					Badmin: '[❗] Eu nem sou administrador Fdp, você acha que sou Deus? Torne-me admin e tente esse comando novamente! ❌',
				...
				case 'demote':
                  ...
                  case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Pedido recebido, adicionar posição como administrador : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
               ...
               if (data.error) return reply(data.error)
				...
				case 'tts':
				   ...
				   if (args.length < 1) return client.sendMessage(from, 'Onde está o código do idioma, tio?', text, {quoted: mek})
					...
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('Ue, Por que você não pede para sua mãe ler tudo isso? Aquela Fela da Puta')
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of groupMembers) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 EAEEE GALERINHA 」*\n*Group* : ${groupName}\n\n${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
               ...
               if (data.error) return reply(data.error)
                ...
                break
                   ...
                   break
					...
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('o recurso está ativo Fdp')
						welkom.push(from)
